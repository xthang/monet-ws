...
export default async function sendTelegramMessage(tag: string, level: 'error' | 'warn' | 'info' | 'log', messages: string[]) {
...
export async function sendTelegramMessageToConversation(tag: string, chatId: string, level: 'error' | 'warn' | 'info' | 'log', messages: string[]) {
    ...
    try {
      ...
      const params = {
        ...
        text: `<b>[${process.env.SERVER_ID}]</b> <b>[${process.env.name ?? '--'}]</b> <b>[${tag}]</b> [<i>${level}</i>] ${messages
          .map((it) => it.replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;').replaceAll('"', '&quot;').replaceAll("'", '&#039;'))
      ...
      if (!resp.ok) console.warnNotSend(`<-- sendTelegramMessage - resp:`, resp.status, '\n| msg:\n', messages, '\n| resp:\n', await resp.text())
    } catch (e) {
