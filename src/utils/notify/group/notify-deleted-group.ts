      ...
      toEmailAddresses.map(({ channel, name, locale, address, ...it }) => {
        ...
        return {
          ...
          subject: contentTemplates.find(
            (it) => it.key === TextTemplateKey.DELETED_GROUP__EMAIL_TITLE && it.locale === locale_
          )!.content,
          text: '',
          html: contentTemplates
            .find((it) => it.key === TextTemplateKey.DELETED_GROUP__EMAIL_CONTENT && it.locale === locale_)!
      ...
      toPhoneNumbers.map(({ channel, name, locale, address, ...it }) => {
        ...
        return {
          ...
          text: contentTemplates
            .find((it) => it.key === TextTemplateKey.DELETED_GROUP__SMS_CONTENT && it.locale === locale_)!
